...
public class P008 {
    ...
    private long product(int offset) {
        return Streams.rangeClosed(offset + 1 - size, offset)
                .mapToLong(i -> numbers.charAt(i) - '0')
                .reduce(1, (m, n) -> m * n);
    }


    public String run() {
        long res = Streams.range(size, numbers.length()).mapToLong(this::product).max().orElseThrow();
